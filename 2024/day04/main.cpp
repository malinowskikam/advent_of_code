...
bool check_mas_direction(std::vector<std::string> &grid, int i, int j,
                         int dir_i, int dir_j);
...
int get_xmas_count_from_pos(std::vector<std::string> &grid, int i, int j) {
    ...
    for (int dir_i = -1; dir_i <= 1; dir_i++) {
        for (int dir_j = -1; dir_j <= 1; dir_j++) {
            if ((dir_i != 0 || dir_j != 0) &&
                check_mas_direction(grid, i, j, dir_i, dir_j)) {
                count++;
            }
        }
    }
...
bool check_mas_direction(std::vector<std::string> &grid, int i, int j,
    ...
    return s_pos_i >= 0 && s_pos_i < grid.size() && s_pos_j >= 0 &&
           s_pos_j < grid[s_pos_i].size() && grid[m_pos_i][m_pos_j] == 'M' &&
           grid[a_pos_i][a_pos_j] == 'A' && grid[s_pos_i][s_pos_j] == 'S';
}
 ...
 */
bool check_x_mas(std::vector<std::string> &grid, int i, int j) {
    if (grid[i][j] != 'A' || i < 1 || i > grid.size() - 2 ||
        j < 1 && j > grid[i].size() - 2) {
        return false;
    }
